...
async function main() {
  ...
  const loadedPathsArr = [...loadedPaths];
  ...
  try {
    ...
    sessionData.retrievedPaths = [...new Set([...sessionData.retrievedPaths, ...loadedPathsArr])];
    // Append signal if detected
    if (signal === 'correction' && loadedPathsArr.length > 0) {
      sessionData.corrections.push({ paths: loadedPathsArr, prompt: prompt.slice(0, 120) });
    } else if (signal === 'approval' && loadedPathsArr.length > 0) {
      sessionData.approvals.push({ paths: loadedPathsArr, prompt: prompt.slice(0, 120) });
    }
